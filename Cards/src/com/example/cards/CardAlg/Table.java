...
public class Table {
	 ...
	 * @param s1 a card bigger than the biggest card on the table except for 7(>=)
	 */
	public void addCard(Card s1) {
		assert isValidAddedCard(s1);
		this.cards.add(s1);
		this.highestValue = CardValue.values.get(s1).intValue();
		this.score += getCardScore(s1);
		assert isValidCards();
	}
